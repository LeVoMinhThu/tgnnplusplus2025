...
if st.session_state.step == 1:
    ...
    if not selected_tickers:
        ...
        with st.spinner("Fetching data... This may take a moment."):
            ...
            # Fetch and save data to disk (not session state)
            yf_raw_data = yf_service.fetch_yahoo_data(selected_tickers, start_date_str, end_date_str)
            yf_raw_data.to_csv(os.path.join(Config.RAW_DATA_DIR, "yf_raw_data.csv"), index=False)

            news_raw_data = news_service.fetch_news_data(selected_news_queries, start_date_str, end_date_str)
            news_raw_data.to_csv(os.path.join(Config.RAW_DATA_DIR, "news_raw_data.csv"), index=False)

            fred_raw_data = macro_service.fetch_fred_data(selected_macro_indicators, start_date_str, end_date_str)
            fred_raw_data.to_csv(os.path.join(Config.RAW_DATA_DIR, "fred_raw_data.csv"), index=False)
        ...
        with st.expander("View Raw Data Samples"):
            ...
            st.dataframe(news_raw_data.head())
            ...
            st.dataframe(fred_raw_data.head())
...
if st.session_state.step == 2:
    ...
    with st.spinner("Processing data..."):
        ...
        # Always read from file, not from session state
        yf_raw_data = pd.read_csv(os.path.join(Config.RAW_DATA_DIR, "yf_raw_data.csv"))
        news_raw_data = pd.read_csv(os.path.join(Config.RAW_DATA_DIR, "news_raw_data.csv"))
        fred_raw_data = pd.read_csv(os.path.join(Config.RAW_DATA_DIR, "fred_raw_data.csv"))

        featured_stock_data = yf_service.featuring_stock_data(yf_raw_data, start_date_str, end_date_str)
        sentimented_news_data = news_service.sentiment_3_class_news_data(news_raw_data)
        ffilled_macro_data = macro_service.ffill_macro_data(fred_raw_data)
...
if st.session_state.step == 3:
    ...
    with st.expander("Correlation Heatmap"):
        ...
        if st.session_state.corr_fig:
            ...
            st.plotly_chart(st.session_state.corr_fig, use_container_width=True, key="corr_fig")
    ...
    for ticker in selected_tickers:
        ...
        # OHLCV Chart
        ohlcv_fig_key = f"ohlcv_fig_{ticker}"
        ohlcv_df_key = f"ohlcv_df_{ticker}"
        if ohlcv_fig_key not in st.session_state or ohlcv_df_key not in st.session_state:
            ohlcv_fig, ohlcv_df = visualize_service.create_ohlcv_fig(final_df, ticker)
            st.session_state[ohlcv_fig_key] = ohlcv_fig
            st.session_state[ohlcv_df_key] = ohlcv_df
        ohlcv_fig = st.session_state[ohlcv_fig_key]
        ohlcv_df = st.session_state[ohlcv_df_key]
        if ohlcv_fig:
            st.caption("OHLCV (Open, High, Low, Close, Volume) candlestick chart.")
            st.plotly_chart(ohlcv_fig, use_container_width=True, key=f"ohlcv_fig_{ticker}")
            chatbot_ui(ohlcv_df, f"ohlcv_{ticker}", chatbot, "OHLCV Chart")

        # Daily Returns Histogram
        returns_fig_key = f"returns_fig_{ticker}"
        returns_df_key = f"returns_df_{ticker}"
        if returns_fig_key not in st.session_state or returns_df_key not in st.session_state:
            returns_fig, returns_df = visualize_service.create_daily_return_histogram(final_df, ticker)
            st.session_state[returns_fig_key] = returns_fig
            st.session_state[returns_df_key] = returns_df
        returns_fig = st.session_state[returns_fig_key]
        returns_df = st.session_state[returns_df_key]
        if returns_fig:
            st.caption("Histogram of daily returns.")
            st.plotly_chart(returns_fig, use_container_width=True, key=f"returns_fig_{ticker}")
            chatbot_ui(returns_df, f"returns_{ticker}", chatbot, "Returns Histogram")

        # Sentiment Line Chart
        sentiment_fig_key = f"sentiment_fig_{ticker}"
        sentiment_df_key = f"sentiment_df_{ticker}"
        if sentiment_fig_key not in st.session_state or sentiment_df_key not in st.session_state:
            sentiment_fig, sentiment_df = visualize_service.create_sentiment_line_chart(final_df, ticker)
            st.session_state[sentiment_fig_key] = sentiment_fig
            st.session_state[sentiment_df_key] = sentiment_df
        sentiment_fig = st.session_state[sentiment_fig_key]
        sentiment_df = st.session_state[sentiment_df_key]
        if sentiment_fig:
            st.caption("Sentiment scores over time.")
            st.plotly_chart(sentiment_fig, use_container_width=True, key=f"sentiment_fig_{ticker}")
            chatbot_ui(sentiment_df, f"sentiment_{ticker}", chatbot, "Sentiment Line Chart")

        # Area Plot
        area_fig_key = f"area_fig_{ticker}"
        area_df_key = f"area_df_{ticker}"
        if area_fig_key not in st.session_state or area_df_key not in st.session_state:
            area_fig, area_df = visualize_service.create_area_plot(final_df, ticker)
            st.session_state[area_fig_key] = area_fig
            st.session_state[area_df_key] = area_df
        area_fig = st.session_state[area_fig_key]
        area_df = st.session_state[area_df_key]
        if area_fig:
            st.caption("Area plot of closing price.")
            st.plotly_chart(area_fig, use_container_width=True, key=f"area_fig_{ticker}")
            chatbot_ui(area_df, f"area_{ticker}", chatbot, "Area Plot")

        # Scatter Close vs Volume
        scatter_fig_key = f"scatter_fig_{ticker}"
        scatter_df_key = f"scatter_df_{ticker}"
        if scatter_fig_key not in st.session_state or scatter_df_key not in st.session_state:
            scatter_fig, scatter_df = visualize_service.create_scatter_close_volume(final_df, ticker)
            st.session_state[scatter_fig_key] = scatter_fig
            st.session_state[scatter_df_key] = scatter_df
        scatter_fig = st.session_state[scatter_fig_key]
        scatter_df = st.session_state[scatter_df_key]
        if scatter_fig:
            st.caption("Scatter plot: Close vs Volume.")
            st.plotly_chart(scatter_fig, use_container_width=True, key=f"scatter_fig_{ticker}")
            chatbot_ui(scatter_df, f"scatter_{ticker}", chatbot, "Scatter Close vs Volume")

        # Doughnut Sentiment
        doughnut_fig_key = f"doughnut_fig_{ticker}"
        doughnut_df_key = f"doughnut_df_{ticker}"
        if doughnut_fig_key not in st.session_state or doughnut_df_key not in st.session_state:
            doughnut_fig, doughnut_df = visualize_service.create_doughnut_sentiment(final_df, ticker)
            st.session_state[doughnut_fig_key] = doughnut_fig
            st.session_state[doughnut_df_key] = doughnut_df
        doughnut_fig = st.session_state[doughnut_fig_key]
        doughnut_df = st.session_state[doughnut_df_key]
        if doughnut_fig:
            st.caption("Doughnut chart of average sentiment.")
            st.plotly_chart(doughnut_fig, use_container_width=True, key=f"doughnut_fig_{ticker}")
            chatbot_ui(doughnut_df, f"doughnut_{ticker}", chatbot, "Doughnut Sentiment")

        # RSI Charts (for each ticker)
        for rsi_type in ["rsi14", "rsi21"]:
            rsi_fig_key = f"rsi_fig_{ticker}_{rsi_type}"
            rsi_df_key = f"rsi_df_{ticker}_{rsi_type}"
            if rsi_fig_key not in st.session_state or rsi_df_key not in st.session_state:
                rsi_fig, rsi_df = visualize_service.create_rsi_figs(final_df, ticker, rsi_type)
                st.session_state[rsi_fig_key] = rsi_fig
                st.session_state[rsi_df_key] = rsi_df
            rsi_fig = st.session_state[rsi_fig_key]
            rsi_df = st.session_state[rsi_df_key]
            if rsi_fig:
                st.caption(f"RSI ({rsi_type.upper()}) for {ticker}.")
                st.plotly_chart(rsi_fig, use_container_width=True, key=f"rsi_fig_{ticker}_{rsi_type}")
                chatbot_ui(rsi_df, f"rsi_{ticker}_{rsi_type}", chatbot, f"RSI ({rsi_type.upper()}) Chart")

        # Moving Average Charts (for each ticker)
        for ma_type in ["ma50"]:
            ma_fig_key = f"ma_fig_{ticker}_{ma_type}"
            ma_df_key = f"ma_df_{ticker}_{ma_type}"
            if ma_fig_key not in st.session_state or ma_df_key not in st.session_state:
                ma_fig, ma_df = visualize_service.create_ma_figs(final_df, ticker, ma_type)
                st.session_state[ma_fig_key] = ma_fig
                st.session_state[ma_df_key] = ma_df
            ma_fig = st.session_state[ma_fig_key]
            ma_df = st.session_state[ma_df_key]
            if ma_fig:
                st.caption(f"Moving Average ({ma_type.upper()}) for {ticker}.")
                st.plotly_chart(ma_fig, use_container_width=True, key=f"ma_fig_{ticker}_{ma_type}")
                chatbot_ui(ma_df, f"ma_{ticker}_{ma_type}", chatbot, f"Moving Average ({ma_type.upper()}) Chart")
    ...
    with st.expander("View Macroeconomic Indicator Charts"):
        for indicator in selected_macro_indicators:
            ...
            if macro_fig:
                st.caption(macro_desc)
                st.plotly_chart(macro_fig, use_container_width=True, key=f"macro_fig_{indicator}")
    ...
    with st.expander("Missing Value Bar Chart"):
        ...
        if st.session_state.missing_fig:
            ...
            st.plotly_chart(st.session_state.missing_fig, use_container_width=True, key="missing_fig")
...
if st.session_state.step == 4:
    ...
    with st.expander("Linear Regression Model"):
        if st.button(f"Train and Evaluate LR for {target_ticker}", key=f"train_lr_button_{target_ticker}"):
            processed_file_path = os.path.join(Config.PROCESSED_DATA_DIR, "final_daily_data.csv")
            try:
                final_df = pd.read_csv(processed_file_path)
            except FileNotFoundError:
        ...
        if "lr_results_df" in st.session_state:
            chatbot_ui(st.session_state.lr_results_df, f"lr_{target_ticker}_stored", chatbot, "Linear Regression Results")
    ...
    with st.expander("Random Forest Model (Predicts Price Change)"):
        if st.button(f"Train and Evaluate RF for {target_ticker}", key=f"train_rf_button_{target_ticker}"):
            processed_file_path = os.path.join(Config.PROCESSED_DATA_DIR, "final_daily_data.csv")
            try:
                final_df = pd.read_csv(processed_file_path)
            except FileNotFoundError:
        ...
        if "rf_results_df" in st.session_state:
            chatbot_ui(st.session_state.rf_results_df, f"rf_{target_ticker}_stored", chatbot, "Random Forest Results")
    ...
    with st.expander("LSTM Model"):
        seq_length = st.slider("Select Sequence Length (days)", min_value=10, max_value=90, value=30, key=f"lstm_seq_{target_ticker}")
        if st.button(f"Train and Evaluate LSTM for {target_ticker}", key=f"train_lstm_button_{target_ticker}"):
            processed_file_path = os.path.join(Config.PROCESSED_DATA_DIR, "final_daily_data.csv")
            try:
                final_df = pd.read_csv(processed_file_path)
            except FileNotFoundError:
        ...
        if "lstm_results_df" in st.session_state:
            chatbot_ui(st.session_state.lstm_results_df, f"lstm_{target_ticker}_stored", chatbot, "LSTM Results")
    ...
    with st.expander("ARIMA Model (Time Series Forecast)"):
        if st.button(f"Train and Forecast ARIMA for {target_ticker}", key=f"train_arima_button_{target_ticker}"):
            processed_file_path = os.path.join(Config.PROCESSED_DATA_DIR, "final_daily_data.csv")
            try:
                final_df = pd.read_csv(processed_file_path)
            except FileNotFoundError:
        ...
        if "arima_results_df" in st.session_state:
            chatbot_ui(st.session_state.arima_results_df, f"arima_{target_ticker}_stored", chatbot, "ARIMA Results")
